 ...
 */
@WebServlet("/employer/ViewAllProjectsServlet")
public class ViewAllProjectsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (usr == null || !(usr instanceof Employer)) {
		...
		out.println("<hr>");
		List<Project> projList = projectDao.list();
		if (!projList.isEmpty()) {
			out.println("<table width='70%' style='border-collapse:collapse; text-align:center; margin-left:auto; margin-right:auto;'");
	        out.println("<tr><th>ID</th><th>Developer Name</th><th>Client Name</th><th>Technology</th><th>Start Date</th><th>Project Status</th><th>Project Manager</th>");
			for(Project proj: projList) {
				...
				out.println("<tr><td>" + proj.getId() + "</td><td>" + proj.getEmployee().getName() + "</td><td>" + proj.getClient() 
					+ "</td><td>" + proj.getTechnology()  + "</td><td>" + proj.getStartDate() + "</td><td>" + showAccept + "</td><td>" + proj.getManager().getName() +" </td>");
			}
			out.println("</table>");
		} else {
